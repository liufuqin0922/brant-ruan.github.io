...
You can use this trick since version 3.6.0 of Octave. That is `Broadcasting` feature. When `==` operating is going, `[1:num_labels]` vector will be currently expand to `m * num_labels` and `y` will be currently expand to `m * num_labels`.
