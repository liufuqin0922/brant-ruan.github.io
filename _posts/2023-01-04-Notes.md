...
需要用mono框架来运行它。运行时需要加32位选项，否则会报错。（另外最好加sudo打开，否则后面很麻烦）

```bash
sudo mono —arch=32 Fiddler.exe
```

https://jingyan.baidu.com/article/22a299b5c4eaae9e18376a45.html

手机设置代理到电脑8888端口，然后在Fiddler中Tools->Options->Connections把Allow remote computers to connect 打勾，然后重启Fiddler才能生效

### Mac上看网页视频

不要安装Flash插件，在Safari开发菜单中选择用户代理为iPad能够解决大部分问题。
...
### Mac OS 安装VMware遇到block问题

http://planetvm.net/blog/?p=3240
...
这样的方法来向文件中写入内容，不需要一直echo。

### Linux下命令行录屏

我的需求是录制视频，而不是用`script`之类做命令回放。发现`Kazam`挺方便的：

```
sudo apt-get install kazam
```

然而录的视频在手机上无法播放，搜了一下，需要转一下码：
...
```
source 打开要转码的视频
video 将 Bitrate 设为2500即可
```

参考[这篇博客](http://blog.csdn.net/ipatient/article/details/51765487)。
